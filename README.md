...
The analogy of a program and a book is a highly valuable one, and extends to many features of both, except when it comes to be able to causally influence the behaviors of machines, which a book cannot. This can be seen as a difference in intent: while a book is written for readers, and must be understandable by human (or at least, excite human understanding, like in Jabberwocky's case<sup>19</sup>) a program is written for a certain machine or a certain compiler that translates it for a certain machine to "understand" it. But even this distinction is not so sharp. In the current engineering world writing programs is a highly social enterprise and the program needs to function not only as a machine instructor but as a mean of communication between humans. Many modern languages try to be as close to natural language as possible, making the code "readable", and many developers emphasize aesthetics in their practices.
